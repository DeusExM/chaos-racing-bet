...
/**
 * Valeurs **remesurées** pour `OVERTAKE_SEED`, dernière fois après l'activation de la forme de fin de
 * course (**P013-cor6**, `GAME_DESIGN` §6.5).
 *
 * La seed est **conservée** : elle produit toujours 11 changements de leader et 37 dépassements (36
 * avant P013-cor6), très au-dessus des minima exigés par les tests E2E, et changer de seed obligerait
 * à remesurer toutes les attentes qui s'y appuient sans rien apporter au jeu. Ce sont donc les
 * **chiffres** qui suivent les règles du noyau, pas l'inverse.
 *
...
/**
 * Seed dont l'arrivée est une **photo finish** (P013), remesurée après P013-cor6.
 *
 * Cherchée sur le corpus canonique de 600 seeds (`observerSeeds.test.ts`) : 77 courses (≈ 12,8 %)
 ...
 * arrivée serrée une fois sur huit est un bon dosage dramatique.
 *
 * `G7Z3JRYQ` remplace `ZH0D03Q4`, qui était l'écart le plus serré **avant** P013-cor6 : la forme de
 * fin de course a redistribué les arrivées, et `ZH0D03Q4` ne sépare plus que `c1` et `c4` de
 * 2,468 m. Le critère n'a pas changé — c'est la course qui a changé —, donc la seed est **remesurée**
 * sur le même corpus : `G7Z3JRYQ` est l'écart P1–P2 le plus serré du corpus **sans être dégénéré**
 * (0,083 m sépare `c2` de `c0`). Aucun gameplay n'a été modifié pour l'obtenir : c'est une course
 * normale du noyau, et `tests/unit/finishModel.test.ts` revérifie l'évidence ci-dessous à chaque
 * exécution.
 */
export const PHOTO_FINISH_SEED = 'G7Z3JRYQ';
 ...
 *
 * `HVEXYB9A` est l'écart P1–P2 le plus large du corpus canonique — 176,56 m à 60 s après P013-cor6,
 * contre 179,33 m avant : la mention « photo finish » n'a aucune ambiguïté à ne pas apparaître ici.
 * Elle remplace `POULET42` dans ce rôle, car l'arrivée de celui-ci est **devenue** une arrivée
 * ordinaire (13,63 m) avec la course de 60 s.
 */
